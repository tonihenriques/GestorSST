...
function NovoRisco(idAtividade, Descricao, AtivId, NomeFuncao, Diretoria, NomeDiretoria) {

    $(".LoadingLayout").show();

    $.ajax({
        method: "POST",
        url: "/TipoDeRisco/NovoRisco",
        data: { idAtividade: idAtividade, Descricao: Descricao, AtivId: AtivId, NomeFuncao: NomeFuncao, Diretoria: Diretoria, NomeDiretoria: NomeDiretoria },
        error: function (erro) {
            $(".LoadingLayout").hide();
            ExibirMensagemGritter('Oops! Erro inesperado', erro.responseText, 'gritter-error');
        },
        success: function (content) {
            ...
            if (content.data !== null) {
                bootbox.dialog({
                    ...
                    title: "<span class='bigger-110'>Cadastrar Riscos</span>",
                    backdrop: true,
                    locale: "br",
                    buttons: {},
                    onEscape: true
